WITH distinct_users AS (
    SELECT distinct * FROM AV_EDM_DEV.ABAGDI_STAGING.stg_confluence_users
), dim_jira_users AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_users
), adp_emp AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_adp_employees
), row_num AS (
