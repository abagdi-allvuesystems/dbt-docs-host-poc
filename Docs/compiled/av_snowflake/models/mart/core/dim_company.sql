WITH dim_sf_account AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_sf_account
), dim_prospect AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_sf_prospect
), dim_oa_client AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_openair_customer
), dim_jira_org AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_organization_mapping
), dist_jira_org AS (
