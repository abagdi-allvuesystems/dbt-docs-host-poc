WITH fact_status AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_CORE.fact_customer_status
), fact_nps_trend AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_CORE.fact_customer_nps_trend
), fact_risk_proj AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_CORE.fact_customer_risk_projection
), fact_support_metrics AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.fact_customer_support_metrics_monthly
    ...
    SELECT * FROM AV_EDM_DEV.ABAGDI_DATA_RIGHTS.fact_customer_data_rights_trend
), fact_csat AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.fact_customer_ticket_survey_monthly
), fact_customer_ps_project_monthly AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_CORE.fact_customer_ps_project_monthly
