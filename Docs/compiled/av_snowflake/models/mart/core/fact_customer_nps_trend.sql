WITH cust AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_CORE.customer
), dim_sf_customers AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_sf_customers
), cust_timeline AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_CORE.customer_timeline
), current_year_forecast AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dimh_sf_account__current_year_risk_forecast
), next_year_forecast AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dimh_sf_account__next_year_risk_forecast
), date_dim AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_date where date >= '1/1/2023' and date <= CURRENT_TIMESTAMP()
), cust_monthly AS (
    ...
    select * from AV_EDM_DEV.ABAGDI_STAGING.dim_sf_nps_surveys
)
