...
WITH dim_date AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_date
), dim_oa_task AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_openair_project_task
), dimh_oa_project AS(
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dimh_openair_project
), ps_project AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_CX_PS.ps_project
), dimh_oa_task AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dimh_openair_project_task
), dim_oa_timeentry AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_openair_time_entries where deleted = false
), dim_oa_task_dates AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_openair_project_task_dates
