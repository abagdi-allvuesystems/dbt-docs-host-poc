...
WITH dim_date AS (
    ...
    SELECT * FROM AV_EDM_DEV.ABAGDI_ENG_ANALYTICS.corp_pi where id = 7
), sprint_workitems as (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_sprint_workitems
), corp_sprints AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ENG_ANALYTICS.corp_sprint
), team_sprints AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ENG_ANALYTICS.team_sprint
