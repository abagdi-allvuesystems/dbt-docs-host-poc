WITH jira_sprint AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_sprints --6311
), agile_team_board AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_litheo_agile_team_boards --28
), corp_sprint AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ENG_ANALYTICS.corp_sprint --104
)
