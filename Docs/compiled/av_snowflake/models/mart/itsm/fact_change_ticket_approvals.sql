WITH jira_app AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_change_approvals
), jira_users AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_users where emailaddress is not null
), change_ticket AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.fact_change_tickets
)
