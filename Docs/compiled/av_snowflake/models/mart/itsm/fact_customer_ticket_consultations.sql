WITH dim_jira_customer_tickets_consultations AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_customer_tickets_consultations
), dim_customer_tickets AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.dim_customer_tickets
), internal_tickets_time_in_status AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_internal_tickets_time_in_status 
), time_in_pending AS (
