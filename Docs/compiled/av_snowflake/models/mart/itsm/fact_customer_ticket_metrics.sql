WITH customer_tickets AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.dim_customer_tickets where resolution_date is not null
), dim_jira_customer_tickets AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_customer_tickets
), customer_tickets_time_in_status AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_customer_tickets_time_in_status 
), time_in_pending AS (
    ...
    FROM AV_EDM_DEV.ABAGDI_STAGING.dimh_jira_jsm_issues_assignee
    group by issue_id
), jsm_worklog AS (
    SELECT *
    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_customer_tickets_worklog
), jsm_worklog_agg AS (
    SELECT issue_id
    ...
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_customer_tickets_first_assignment_metric
), time_to_engage AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_customer_tickets_time_to_engage_metric
)
