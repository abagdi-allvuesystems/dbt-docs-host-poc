WITH dim_jira_change_tickets AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_change_tickets
), jsm_approvals AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.jira_jsm_issue_approvals
), jsm_approvers AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.jira_jsm_issue_approval_approvers
), full_set AS (
