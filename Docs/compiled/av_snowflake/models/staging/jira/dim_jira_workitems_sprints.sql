...
WITH workitems_current AS (
    ...
    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_workitems
), sprint_workitems_current AS (
    SELECT *

    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_sprint_workitems_current
...
GROUP BY issue_id
    ...
    FROM AV_EDM_DEV.ABAGDI_STAGING.jira_issue_changelog_sprints
