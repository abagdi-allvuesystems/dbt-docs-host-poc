...
WITH issue AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_issue WHERE issue_category_id != 2 AND deleted != TRUE
), issuecat AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_issue_category
), issuesev AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_issue_severity
), issuesource AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_issue_source
), issuestage AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_issue_stage
), issuestatus AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_issue_status
), customer AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_openair_customer
), user AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_openair_user
), project AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_openair_project
)
