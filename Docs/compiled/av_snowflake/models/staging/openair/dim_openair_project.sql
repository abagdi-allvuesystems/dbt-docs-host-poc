WITH oa_proj AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_project WHERE deleted = FALSE 
),dimh_proj AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dimh_openair_project where is_current = true
), first_last_time_entry AS (
    select p.id
        ,min(time_entry_date) as first_time_entry
        ,max(time_entry_date) as last_time_entry
    from oa_proj p JOIN AV_EDM_DEV.ABAGDI_STAGING.dim_openair_time_entries t on p.id = t.project_id
    group by p.id
),oa_project_stage AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_project_stage
), oa_cust AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_customer
), oa_user as (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_user
), oa_original_budget as (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_openair_budget where is_initial = true
), oa_project_owner AS (
    SELECT op.id   
            ,op.user_id
            ,ou.name
    ...
    FROM AV_EDM_DEV.ABAGDI_STAGING.dimh_openair_project
