...
WITH oa_legacy_snap as (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dimh_openair_project_snapshot_rpt 
--), dbt_oa_proj_snap AS (
--    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_project_snapshot 
), oa_project_stage as (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_project_stage
), oa_project AS (
    SELECT id
        ,name
    FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_project
), all_snaps as (
