WITH sf_cust AS (
    ...
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.sys_sf_account
), assets_orgs AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_assets_organizations
), distinct_client_services AS (
    SELECT DISTINCT CLIENT FROM AV_EDM_DEV.ABAGDI_STAGING.dim_assets_ism_client_services WHERE CLIENT IS NOT NULL
    ...
    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_sf_contract_msa 
    WHERE STATUS = 'Activated'
    GROUP BY account_id
), accts_with_active_pr AS (
    SELECT account_id AS sf_account_id
           ,COUNT(contract_number) as count_active_product_riders 
    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_sf_contract_product_rider 
    WHERE STATUS = 'Activated'
    GROUP BY account_id
