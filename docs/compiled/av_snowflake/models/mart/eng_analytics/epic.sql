WITH jira_epic AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_epics
), jira_epic_pis AS (
    SELECT 
            issue_id
            ,LISTAGG(PI_VALUE,',') AS jira_pi

    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_epic_pi_new
    GROUP BY issue_id
), associated_products AS (
    SELECT issue_id
        ,LISTAGG(associated_products_field_value,',') AS associated_products
    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_associated_products
    GROUP BY issue_id
), market_segment AS (
    SELECT issue_id
        ,LISTAGG(market_segement_field_value,',') AS market_segment
    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_market_segment
    GROUP BY issue_id
), aha_feature AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_aha_features
), aha_features_pis AS (
    SELECT 
            FEATURE_ID
            ,LISTAGG(VALUE,',') AS aha_pi

    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_aha_features_pi_planning
    GROUP BY FEATURE_ID
