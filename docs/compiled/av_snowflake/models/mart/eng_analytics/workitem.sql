WITH jira_workitems AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_workitems
), jira_workitems_sprints_current as (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_sprint_workitems
    WHERE effective_date_end = '9999-12-31'
), jira_sprints AS (
    SELECT DISTINCT id,start_date,name FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_sprints
    ...
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.epic_id_mapping
), solution_project_mapping AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.solution_project_id_mapping
), wi_fix_vers AS (
    SELECT * from AV_EDM_DEV.ABAGDI_ENG_ANALYTICS.workitem_fix_versions
), release_vers AS (
    SELECT * from AV_EDM_DEV.ABAGDI_ENG_ANALYTICS.release_versions
    ...
    FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_epics
)
