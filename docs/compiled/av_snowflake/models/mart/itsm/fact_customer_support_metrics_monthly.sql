WITH fact_ticket_backlog AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.fact_customer_ticket_backlog
), core_cust AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_CORE.customer
), jira_org_map AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.jira_organization_mapping
), cust_tickets AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.customer_tickets
), cust_tickets_cust AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.customer_tickets_customers
), dim_date AS (
