WITH dim_jira_surveys AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_customer_surveys
), dim_sf_nps_surveys AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_sf_nps_surveys
), dim_customer_tickets AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.dim_customer_tickets
), dim_customer_tickets_customers AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_ITSM.dim_customer_tickets_customers
), dim_customer_system_mapping AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_av_customer_system_mapping
)
