WITH sys_jira_issues_linked AS (
    SELECT * 
    FROM AV_EDM_DEV.ABAGDI_SYSTEM.jira_jsm_issue_issuelinks
    WHERE direction_name = 'is caused by'
), cust_tickets_incidents AS (
    SELECT *
    FROM AV_EDM_DEV.ABAGDI_ITSM.dim_customer_tickets
    WHERE ticket_type = 'Incident'
), change_tickets AS (
    SELECT *
    FROM AV_EDM_DEV.ABAGDI_ITSM.fact_change_tickets
)
