WITH features AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.aha_features
), custom AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.aha_features_custom_fields
), integration as (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.aha_features_integration_fields
)
