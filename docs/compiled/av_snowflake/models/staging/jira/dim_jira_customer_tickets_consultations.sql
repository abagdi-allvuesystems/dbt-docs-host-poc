WITH jira_jsm_issues AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.jira_jsm_issues
), jira_issues AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.jira_issues
), dim_jira_customer_tickets AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_jira_customer_tickets
), jira_jsm_issue_issuelinks AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.jira_jsm_issue_issuelinks
)
