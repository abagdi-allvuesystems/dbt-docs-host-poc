WITH portal_users AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.jira_portal_only_customers
), managed_users AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.jira_managed_users
), users_all AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.jira_users
)
