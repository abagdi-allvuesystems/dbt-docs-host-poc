WITH hist_dim_task AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dimh_openair_project_task where is_current = true
), sys_proj_task AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_project_task 
), product_xref AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_STAGING.dim_litheo_product_xref
), sys_task_type AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_project_task_type
)
