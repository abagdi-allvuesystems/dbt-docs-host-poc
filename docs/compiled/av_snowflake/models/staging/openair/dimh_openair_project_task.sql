...
WITH oa_proj_task_tasks AS (
    SELECT id, custom_352 AS scope_hours FROM AV_EDM_DEV.ABAGDI_SYSTEM.openair_project_task where is_a_phase = false
), oa_proj_task_snap AS (
    SELECT * FROM av_edm_snapshots_dev.openair.openair_project_task_snapshot 
)
