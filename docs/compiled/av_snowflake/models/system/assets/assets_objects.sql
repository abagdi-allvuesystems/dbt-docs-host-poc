WITH assets_itsm AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SOURCE.ab_itsm_objects
), assets_ism AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SOURCE.ab_ism_objects
), assets_services AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SOURCE.ab_services_objects
)
, union_raw AS (
    SELECT id
            ,objecttype
