WITH objectschema_attr AS (
    SELECT * FROM AV_EDM_DEV.ABAGDI_SOURCE.ab_objectschema_attributes
), assets_type_attribute AS (
    SELECT DISTINCT o.objecttype_id,a.objecttypeattributeid
    FROM AV_EDM_DEV.ABAGDI_SYSTEM.assets_objects o JOIN AV_EDM_DEV.ABAGDI_SYSTEM.assets_objects_attributes a ON o.id = a.objectid
)
SELECT attr.id
